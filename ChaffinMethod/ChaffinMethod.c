...
// Uncomment this for systems using CRLF for files.
...
//	Compute 10^(n-1)
    ...
    nfactor=BASE;
    for (int k=0; k<n-2; k++) nfactor*=BASE;
...
//	maxDecM is the equivalent for (n-1)-digit sequences
    ...
    maxDecM = n;
    for (int k=0; k<n-2; k++) maxDecM = BASE*maxDecM + n;
    maxDec = BASE*maxDecM+n;
    maxDec++;
    maxDecM++;
...
//	Check for any pre-existing files
    ...
    for (tot_bl=1; tot_bl<maxW; tot_bl++)
    {
        ...
        bestLen[tot_bl] = len;
#ifdef CRLF
        nBest[tot_bl] = (int)(fsize/(len+2));
        if (fsize != nBest[tot_bl]*(len+2))
        {
            printf("Size of file %s is %ld, which is not exactly divisible by line length %d\n",outputFileName,fsize,len+2);
            exit(EXIT_FAILURE);
        };
#else
...
//	These have been sorted into increasing order of ldd[tperm]
    ...
    for	(int z=0; z<n-1; z++)
    {
        ...
        if (newperm)
        {
            ...
            fillStr(pos+1, pfound+1, tperm/BASE, TRUE);
        ...
        else if	(spareW > 0)
        {
            if (vperm)
            {
                ...
                {
                    fillStr(pos+1, pfound, tperm/BASE, TRUE);
                };
            }
            else
            {
                ...
                {
                    fillStr(pos+1, pfound, tperm/BASE, FALSE);
                }
                else return;
            };
        };
...
// there is never any benefit to having 2 of the same character next to each other
    ...
    if	(j1 != curstr[pos-1])
    {
        ...
        if (newperm)
        {
            ...
            unvisited[tperm]=FALSE;
            fillStr2(pos+1, pfound+1, tperm/BASE, swap, bestStr+1, len-1);
            unvisited[tperm]=TRUE;
        ...
        else if	(alreadyWasted < tot_bl)
        {
            ...
            {
                fillStr2(pos+1, pfound, tperm/BASE, swap, bestStr+1, len-1);
            };
        };
    };
}
