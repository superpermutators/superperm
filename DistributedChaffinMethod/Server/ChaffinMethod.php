			...
			if ($row['status']=='A') {
				...
				if (substr($str,0,$pref_len)==$pref) {
					...
					if (analyseString($str,$n) > 0) {
						...
						$res = $pdo->prepare("INSERT INTO tasks (original_task_id, access,n,waste,prefix,perm_to_exceed,status,branch_order,prev_perm_ruled_out,iteration,ts_allocated,ts_finished,excl_witness,checkin_count,perm_ruled_out,client_id) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
