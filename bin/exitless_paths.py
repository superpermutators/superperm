...
def get(S,E,cutoff):
    todo = {}   #keeps track of the paths we need to check out, and try to continue further.
                #the key corresponds to the current end of the path, and the value of the key is the set of such paths with that end
    done = {}   #keeps track of current least wastage to get to reach states.
                #it's key is also the end of the path, which yeilds another dicitonary
                #the second dicitonary is we give a certain wastage, which returns the set of states with that end which are known to have this wastage

    for start,cost in S: #builds todo and done
        if cost > cutoff:
            continue
        end = start.pop(-1) #we always pop off our last vertex, that's where we start when we continue our path
        if end not in done:
    ...
    while todo:
        ...
        while queue:
            ...
            for cost0 in done[front]:
                ...
                options -= ops
                #it would be nice to discard the part of ops which has already been checked with the specific front but I haven't gotten aroudn to it.
...
def findbests(done): #returns the length of the longest path of each 1-cycle skippage.
    best = [0]*100
    for end in done:
...
def add(e, front): #we start are path at front, and then move places according to e
    out = [front]
    for step in e:
...
#m is the indices you move around with your edge
#since we always complete our 1-cycles in exitless paths, the last letter of your permutation will end up as the first letter when we're actually doing this move.
def move(p,m):
    x = list(p)
    y = []
    for i in m:
...
def reduce(seq): #converts the rearrangement of genmoves for the move function
    out = list(seq)
...
def genmoves(n): #create all n-edges
    return [reduce(a) for a in it.permutations(list(range(n-1))+[-1])]


tau = phi([0,-1]) #2-edge

Ts = [([0],k-2)] #how we start our path, before our first 3-edge. ([visited vertices], defined wastage)
for i in range(k-2):
    Ts.append((Ts[-1][0]+[tau[Ts[-1][0][-1]]],Ts[-1][1]-1))

moves = []

for m in genmoves(3): #how we will continue our path, consists of a 3-edge, followed by a number of 2-edges
    toadd = [((phi(m),),k-2)] #(tuple of maps, defined wastage), we use the sequence of maps with the "add" function
    for _ in range(k-2):
        toadd.append((toadd[-1][0]+(tau,),toadd[-1][1]-1))
        if len(cycs(add(toadd[-1][0],0))) < len(toadd[-1][0])+1: #checks if we re-entered a 1-cycle
            toadd.pop(-1)
            break
